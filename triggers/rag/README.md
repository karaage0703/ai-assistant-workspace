...
`skills/xs-workspace-rag/` の常駐サーバー（HTTP API）に問い合わせる薄いラッパー。RAGをセットアップした利用者が、LLMのFunction Callingまたはシェルから `bash triggers/rag/handler.sh ...` で呼び出す。

RAGは任意機能。初期状態ではこのtriggerを自動実行しない。
...
LLMから呼ぶ場合は、RAGを利用するよう明示された依頼に対して適切な引数を組み立てる。
...
セットアップ後は、RAGを使うよう明示した依頼から利用できる。
