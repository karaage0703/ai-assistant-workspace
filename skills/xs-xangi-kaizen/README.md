...
AIアシスタント上で起きた事象（再投稿、cron が回らない、データ不整合、想定外の動作、エラー等）を調査し、証拠・原因・対策案を報告するスキル。修正・設定変更・再起動は、ユーザーが変更まで依頼した場合だけ行います。
...
- 明示された範囲での修正・検証。詳細経緯はノート、短い判断基準は行動ルールへ整理する案を提示
