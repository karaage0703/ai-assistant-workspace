...
- 初回利用・セットアップを変える場合は、既存の認証や事前データに依存しない隔離環境で、初期状態から目的を達成できることを確認する。本番設定を消して試さない。
- ローカルでは変更箇所のテスト・短い静的検査・実機確認を行い、全体の回帰はCIへ任せる。CIがない場合や失敗再現が必要な場合はローカルで補う。同じ成功済み検査を理由なく繰り返さない。
- 作業ディレクトリごとにlockfileに従って依存を導入する（例: `npm ci` / `uv sync --frozen`）。別checkoutの依存ディレクトリを共有して検証済みとしない。
- lint・test・buildは対象リポジトリで定義されたコマンドを使う。
...
マージ、稼働環境への配備、サービス再起動はそれぞれ別操作として、ユーザーが許可した範囲だけ実行する。まとめて明示許可された場合は再確認しない。

- マージ前: 対象PRの最新HEAD・検証結果・未解決指摘を確認する。
- 配備前: 稼働checkoutと反映対象、設定・依存関係を確認する。
- 再起動後: 実際のプロセス状態、反映された版、疎通を確認する。
- 報告例: 「PRはマージ済み。配備・再起動は未実施」。コード統合だけで本番反映済みとは報告しない。
