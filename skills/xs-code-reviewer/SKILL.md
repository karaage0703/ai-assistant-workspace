...
description: 複数のAI（Claude/Codex）でPRをレビューするスキル。GitHub PRの差分を取得し、各AIの視点を統合した詳細レビューを提供。「PRレビューして」「コードレビューして」「PR#123をチェック」で使用。
---
...
**マルチエージェント機能**: Codex CLI が利用可能な場合、作成したレビューコメントの妥当性を検証してもらい、より高品質で正確なレビューを実現します。
...
Codex CLI が利用可能な場合、以下のフローでレビューを実行：
...
レビューコメントを投稿する前に、Codex で妥当性を検証します。
...
- Codexが利用できない場合は、Claude単独でレビューを実施
...
- Codex の完了を待ってから結果統合
- タイムアウト: 120秒
- Codex が失敗した場合は Claude 単独でレビューする
